...
export class Shelves extends Component {
  ...
  reset(contents, options) {
    ...
    const { isEndUnit, frameHeight, shelfYOffset, frame, chassisType } = options;
    const gasSping =
      data &&
      data.handle &&
      (data.handle.swing === 'top' || data.handle.swing === 'bottom');
    const shelfSpacing = frameHeight > 24 ? 2 : 1;
    const numOfSlots = Math.floor((boundry.height - 7) / shelfSpacing) + 5;
    ...
    const heightDiff = boundry.height - CART_CENTER_PANEL_MESH_HEIGHT

    const chassisYAdj = (!chassisType || chassisType === 'mini' || chassisType === 'standard') ? -0.8 : 0;
    const firstSlotY = -3.25 - (heightDiff/2) + chassisYAdj;

    let bracingWidthAdj = 0.0;
    let bracingXAdj = 0;
    if(frame.left === 'corner post'){
      bracingWidthAdj -= 0.6;
      bracingXAdj += 0.3;
    }
    if(frame.right === 'corner post'){
      bracingWidthAdj -= 0.6;
      bracingXAdj -= 0.3;
    }
    ...
    let actualWidth =
      ...
      bracingWidthAdj;
    let x = (extensionLength / 2) * (leftExt ? -1 : 1) + bracingXAdj;
      ...
      const shelfAsset = {
        ...
        data: {
          ...
          depth:
            boundry.depth -
            (CART_SHELF_DEPTH_ADJUSTMENT +
              (gasSping ? CART_SHELF_DEPTH_ADJUSTMENT : 0)),
          x: x,
          y: firstSlotY + indexOfSlot * shelfSpacing,
          z:
            -SHELF_OFFSET_DEPTH +
            (gasSping ? 0 : CART_SHELF_DEPTH_ADJUSTMENT / 2),
          slotDepthSpacing,
          clipZAdjustment:
...
function calcSlotDepthSpacing(depth){
  return depth - 8.51;
}